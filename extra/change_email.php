...
{
	$count = 0;
	$stm = $db->prepare("SELECT link FROM change_email WHERE link = ?");
	...
	{
		$status = 0;
		$stm = $db->prepare("SELECT link FROM change_email WHERE CURRENT_TIMESTAMP > start_time AND CURRENT_TIMESTAMP < end_time AND link = ?");
		$stm->bind_param("s",$var);
		$stm->execute();
		$stm->store_result();
		$status = $stm->num_rows;
...
if(isset($_POST["login"]))
{
	...
	$stm = $db->prepare("SELECT link FROM change_email WHERE link = ? AND CURRENT_TIMESTAMP > start_time AND CURRENT_TIMESTAMP < end_time");
	$stm->bind_param("s",$var);
	$stm->execute();
	$stm->store_result();
	$count = $stm->num_rows;
	...
	if($count == 1)
	{	
		$stm = $db->prepare("SELECT username FROM change_email WHERE link = ? ");
		$stm->bind_param("s",$var);
		$stm->execute();
		...
		$stm = $db->prepare("SELECT new_email FROM change_email WHERE link = ? ");
		$stm->bind_param("s",$var);
		$stm->execute();
		$new_email = $stm->get_result()->fetch_object()->new_email;
		$stm->free_result();
		
		if (password_verify($password, $passworddb)) {
			//update the email login
			$flag = 0;
			if (($stmt = $db->prepare("UPDATE users_login SET email = '".$new_email."' WHERE oauth_uid = ? "))) {
				if ($stmt->bind_param("s",$username)) {
						if ($stmt->execute()) {
							//now delete the entry
							$flu = 0;
							if (($stmt = $db->prepare("DELETE FROM change_email WHERE username = ? AND link = ?"))) {
...
<div class="section-background top-0">
	...
	<div class="container" style="font-size: 18px">
		<h1 style="font-size: 30px"><strong>Change Email !</strong></h1><br/>
		 <div>
	...
	';
		...
		<div class="container" style="font-size: 18px">
		<h1 style="font-size: 30px"><strong>Change Email !</strong></h1><br/>
		<h2>You\'re link has been <span style="color:red">expired</span> please generate a new link and don\'t forget to activate it in 3 hours !</h2>
	</div>
		';
	?>
