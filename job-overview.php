...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true && isset($id)) {
	...
	$exp = 0;
	$stm = $db->prepare("SELECT id FROM jobs WHERE id = ? AND CURRENT_DATE <= expiration AND admin_verified = '1' ");
	$stm->bind_param("s",$id);
	$stm->execute();
	$stm->store_result();
	$exp = $stm->num_rows;
	...
	$count = 0;
	$stm = $db->prepare("SELECT id FROM jobs WHERE id = ? AND admin_verified = '1' ");
	$stm->bind_param("s",$id);
	$stm->execute();
	$stm->store_result();
	$count = $stm->num_rows;
	...
	if($count == 1){
		//fetch job details
		$stm = $db->prepare("SELECT * FROM jobs WHERE id = ?");
		$stm->bind_param("s",$id);
		$stm->execute();
		$res = $stm->get_result();
...
if(isset($_POST['apply']))
{
	...
	if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true){
		...
		if($account == 'user'){
			...
			if($expp == 1){
				...
				//now do the same with jobs section
				$jobs_arrays = json_decode($rowwed['applied_id']);
				$jobs_count = count($jobs_arrays);
				for($i=0;$i<$jobs_count;$i++)
					$jobs_array[$i] = $jobs_arrays[$i][0];
				if(!empty($jobs_arrays)){
					if(!(in_array($userid, $jobs_array))){
						 //now push a new element
						$idm = [(string)$userid, "0"];
						array_push($jobs_arrays,$idm);
					}
					else
						$err = $err . "You have Already Applied for the Job !";
				}
	...
	if($err == ""){
		...
		if (($stmt = $db->prepare("UPDATE jobs SET applied_id = ? WHERE id = ?"))) {
			if ($stmt->bind_param("ss",json_encode($jobs_arrays),$id)) {
				if ($stmt->execute()){
					if(($stmy = $db->prepare("UPDATE users_profile SET jobs = ? WHERE username = ?"))){
						if($stmy->bind_param("ss",json_encode($user_arrays),$_SESSION['userData'])){
							if($stmy->execute()){
								header("Location: job-overview.php?id=".$id."");
							}
							else
								$flag = 1;
						}
...
<meta property="og:url"   content="http://careerstairs.in/job_overview.php?id=<?php echo $id ?>" />
<meta property="og:type"  content="website" />
<meta property="og:title" content="<?php echo $rowwed['job_title'] . ' job in ' .  $rowwed['company_name']  ?>" />
<meta property="og:description" content="<?php echo $rowwed['job_details']; ?>" />
<meta property="og:image"       content=<?php echo ' "http://careerstairs.in/dashboard/company_pictures/'.$rowwed['company_pic'].'" '; ?> />
...
<div class="container">
	...
	<div class="padding-right">
		...
		<div class="company-info">
			<?php echo '<img src="dashboard/company_pictures/'.$rowwed['company_pic'].'" alt="'.$rowwed['company_name'].'">'; ?>
			<div class="content">
				<h4><?php echo $rowwed['company_name'] ?></h4>
				<span><a href="#"><i class="fa fa-link"></i> <?php echo $rowwed['company_website'] ?></a></span>
			</div>
			<div class="clearfix"></div>
		</div>
		...
		<p class="margin-reset" style="text-align: justify">
			<?php echo $rowwed['company_description']; ?>
		</p>
		<br/>
		...
		<div class="widget">
			...
			<div class="job-overview">
				...
				<ul>
					<li>
						...
						<div>
							...
							<span><?php echo dateformat($rowwed['expiration']) ?></span>
						</div>
					</li>
					<li>
						...
						<div>
							...
							<span><?php echo implode(" , ",json_decode($rowwed['location'])) ?></span>
						</div>
					</li>
					<li>
				...
				if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true){
					...
					//if not expired
					if($exp == 1)
						echo '<a style="pointer-events: none;cursor: default;" href="login.php" class="popup-with-zoom-anim button">Login to Apply</a>';
					else
						echo '<a class="button" style="background-color: grey;pointer-events: none;cursor: default;">Expired</a>';
				}
...
document.getElementById('shareBtn').onclick = function() {
  FB.ui({
    method: 'share',
    mobile_iframe: true,
    href: 'http://careerstairs.in/job-overview.php?id=<?php echo $id ?>',
  }, function(response){});
}
</script>
